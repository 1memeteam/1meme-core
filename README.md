[Website](https://1.meme/) | [X](https://x.com/one__meme) | [WhitePaper](https://Bonding.io/papers/Whitepaper_BondingLabs.pdf)
...
```
git clone https://github.com/1memeteam/1meme-core.git
git checkout v1.2
...
├─ src
│  ├─ MeMe1TokenFactory.sol // the preset will be cloned inside the factory
│  ├─ abstract
│  │  ├─ MeMe1Base.sol  //the basic bonding curve logic
│  ├─ bondingCurve //the bonding curve model
│  ├─ hooks //some optional hooks can inject when the Bonding token is minting, burning or transferring
│  ├─ interfaces
│  ├─ preset //the extended preset base on MeMe1Base
│  │  ├─ MeMe1ERC20Mixed.sol //default Bonding ERC20 implement, with infinite supply
│  │  ├─ MeMe1ERC20WithSupply.sol //the extension of MeMe1ERC20Mixed, with specific supply
│  │  ├─ MeMe1LOL.sol //basic LOL implement, extended from MeMe1ERC20WithSupply
│  │  └─ MeMe1LOLX1.sol //the X1 Chain implement of LOL
├─ test //unit test
...
```
 forge install github.com/1memeteam/1meme-cor
```
...
import "Bonding-contracts/src/interfaces/IMeMe1Factory.sol";
import "Bonding-contracts/src/interfaces/IMeMe1Token.sol";


contract BondingIntegrate{
    IMeMe1Token currentToken;
    //deploy a new token
    function deployToken() external {
        ...
        hooks[0] = address(0x3);//the address of hook, you can find it from our graph api. https://github.com/BondingProtocol/Bonding-graph.git
        ...
        currentToken = IMeMe1Token(factory.deployTokenWithHooks(info, 0, hooks, datas));
    }

    function mintAndBurn() external {

        //deposit exact raising token to mint Bonding token
        ...
        //deposit raising token to mint exact Bonding token
        ...
        //burn exact Bonding token to refund raising token
        uint256 tokenToBurn
