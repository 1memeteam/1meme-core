...
contract ExpMixedTest is BaseTest {
    ...
    function testEstimateMintBurn() public {
        ...
        vm.expectRevert();
        currentToken.mint(user1, 1000 ether, receivedAmount + 1);
        currentToken.mint(user1, 1000 ether, receivedAmount);
    ...
    function testMultiMint() public {
        ...
        for (uint256 i = 0; i < round; i++) {
            currentToken.mint(user1, 1000 ether, 0);
            uint256 platformBalance = fakeUSDT.balanceOf(platformTreasury);
            uint256 treasuryBalance = fakeUSDT.balanceOf(projectTreasury);
            uint256 tokenBalance = fakeUSDT.balanceOf(address(currentToken));
            uint256 userBalance = fakeUSDT.balanceOf(user1);
            uint256 erc20Balance = currentToken.balanceOf(user1);
    ...
    function testMultiBurn() public {
        ...
        fakeUSDT.mint(1e8 ether);
        fakeUSDT.approve(address(currentToken),1e8 ether);
        currentToken.mint(user1, 500000 ether, 0);
    ...
    function testRandomMintAndBurn() public {
        ...
        for (uint256 i = 0; i < round; i++) {
            ...
            vm.startPrank(user1);
            fakeUSDT.mint(amount1);
            fakeUSDT.approve(address(currentToken),amount1);
            currentToken.mint(user1, amount1, 0);
            // user 2 mint or burn random amount
            uint256 amount2 = ((randomSeed % 999) + 1) * 0.1 ether;
            vm.startPrank(user2);
            fakeUSDT.mint(amount2);
            fakeUSDT.approve(address(currentToken),amount2);
            currentToken.mint(user2, amount2, 0);
            ...
            vm.startPrank(user3);
            if (randomSeed % 2 == 1) {
                fakeUSDT.mint(amount3);
                fakeUSDT.approve(address(currentToken),amount3);
                currentToken.mint(user2, amount3, 0);
            } else {
                fakeUSDT.mint(amount3);
                fakeUSDT.approve(address(currentToken),amount3);
                currentToken.mint(user3, amount3, 0);
            }
            if (randomSeed % 10 < 2) {
                ...
                if (user3Erc20 >= 0.1 ether) {
                    ...
                    uint256 treasuryBalance = fakeUSDT.balanceOf(projectTreasury);
                    uint256 tokenBalance = fakeUSDT.balanceOf(address(currentToken));
                    uint256 user1Balance = fakeUSDT.balanceOf(user1);
                    uint256 erc20Balance1 = currentToken.balanceOf(user1);
                    uint256 user2Balance = fakeUSDT.balanceOf(user2);
                    uint256 erc20Balance2 = currentToken.balanceOf(user2);
                    uint256 user3Balance = fakeUSDT.balanceOf(user3);
    ...
    function testFuzz() public {
        vm.startPrank(user1);
        uint256 amount = 1e8 ether;
        console.log(amount);
        fakeUSDT.mint(amount);
        fakeUSDT.approve(address(currentToken), amount);
        currentToken.mint(user1, amount, 0);
        uint256 balanceBefore = fakeUSDT.balanceOf(address(currentToken));
        console.log(currentToken.circulatingSupply());
        currentToken.burn(user1, currentToken.circulatingSupply(), 0);
        console.log(fakeUSDT.balanceOf(address(currentToken)), balanceBefore);
        console.log(currentToken.circulatingSupply());
    }
