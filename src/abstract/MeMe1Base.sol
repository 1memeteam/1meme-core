...
import "./MeMe1Metadata.sol";
import "../interfaces/IMeMe1Factory.sol";
import "../interfaces/IHook.sol";
import "../interfaces/IERC20WithDecimals.sol";

abstract contract MeMe1Base is MeMe1Metadata, SwapCurve, AccessControlUpgradeable, ReentrancyGuard {
    ...
    IMeMe1Factory internal _factory;
    ...
    /// @inheritdoc IMeMe1Token
    function initialize(address bondingCurveAddress, IMeMe1Factory.TokenInfo memory token, address factory) public virtual {
    ...
    /// @inheritdoc IMeMe1Token
    function renounceAdmin() public onlyRole(PROJECT_ADMIN_ROLE) {
        _revokeRole(PROJECT_ADMIN_ROLE, _projectAdmin);
        _projectAdmin = address(0);
        emit LogProjectAdminChanged(address(0));
    }

    function setProjectTaxRateAndTreasury(address newProjectTreasury, uint256 projectMintTax, uint256 projectBurnTax) public onlyRole(PROJECT_ADMIN_ROLE) {
    ...
    function _initFactory(address account) internal {
        ...
        _factory = IMeMe1Factory(account);
    }
    ...
    /// @inheritdoc IMeMe1Token
    function setProjectTaxRate(uint256 projectMintTax, uint256 projectBurnTax) public virtual onlyRole(PROJECT_ADMIN_ROLE) {
        _setProjectTaxRate(projectMintTax, projectBurnTax);
    }
    ...
    function mint(address to, uint payAmount, uint minReceive) public payable virtual nonReentrant {
        ...
        for (uint256 i = 0; i < hooks.length; i++) {
            IHook(hooks[i]).beforeMintHook(to, tokenAmount, payAmountActual);
        }
        ...
        for (uint256 i = 0; i < hooks.length; i++) {
            IHook(hooks[i]).afterMintHook(to, tokenAmount, payAmountActual);
        }
    ...
    function burn(address to, uint payAmount, uint minReceive) public payable virtual nonReentrant {
        ...
        address[] memory hooks = getHooks();
        for (uint256 i = 0; i < hooks.length; i++) {
            IHook(hooks[i]).beforeBurnHook(from, payAmount, amountReturn);
        }
        ...
        for (uint256 i = 0; i < hooks.length; i++) {
            IHook(hooks[i]).afterBurnHook(from, payAmount, amountReturn);
        }
